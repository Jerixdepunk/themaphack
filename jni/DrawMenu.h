...
void HideMenu(bool& bShow) {
    ...
    if (ImGui::BeginPopupModal("ConfirmHide", 0, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse)) {
        ...
        if (ImGui::Button("Yes", ImVec2(ImGui::GetContentRegionAvail().x / 2, 0))) { showMenu = false; bShow = false; ImGui::CloseCurrentPopup(); }
        ImGui::SameLine();
        if (ImGui::Button("No", ImVec2(ImGui::GetContentRegionAvail().x, 0))) { bShow = false; ImGui::CloseCurrentPopup(); }
        ImGui::EndPopup();
    }
}
...
struct sTheme { bool TemaAsik; bool Color; };
sTheme Theme{0};

std::string msg;
void LoginThread(const std::string &user_key, bool *success) { msg = Login(g_vm, user_key.c_str(), success); }
bool selectedThemes;
...
inline ImColor GetColorWithAlpha(ImColor color, float alpha) { return ImColor(color.Value.x, color.Value.y, color.Value.z, alpha); }
inline ImVec2 center_text(ImVec2 min, ImVec2 max, const char* text) { return min + (max - min) / 2 - ImGui::CalcTextSize(text) / 2; }
...
void DrawMenu() {
    ...
    if (!isLogin) {
        ...
        // (Everything here is your "else" block - Keep your original code for the tabs here)
...
// ============================================================
// 🚀 FULLY REWRITTEN DrawMenu - NO LOGIN REQUIRED!
// ============================================================
void DrawMenu() {
	const ImGuiViewport* main_viewport = ImGui::GetMainViewport();
	ImVec2 center = ImGui::GetMainViewport()->GetCenter();
    ...
    // Load battle data immediately (no login required)
    loadBattleData(battleData);

    std::string XYOURZONE = std::string("FREE VERSION ");
	...
	// =============================================
    // MAIN MENU - Login completely removed!
    // =============================================
	if (ImGui::BeginTabBar("Tab", ImGuiTabBarFlags_FittingPolicyScroll)) {
		// -------- ESP TAB --------
		if (ImGui::BeginTabItem("ESP")) {
		...
		// -------- SETTING TAB (Login info REMOVED) --------
		if (ImGui::BeginTabItem("Setting")) {
            ImGui::BeginGroupPanel("Menu Setting", ImVec2(-1.0f, 0.0f));
            {
                ImGui::Checkbox("Auto Resize", &bFlagAutoResize);
                ...
                // Removed: Key Info (expired, slotZ, VIP status)
                ImGui::TextColored(RGBA2ImVec4(0, 255, 0, 255), "FREE VERSION - No login required!");
            }
            ImGui::EndGroupPanel();
            ImGui::EndTabItem();
        }
